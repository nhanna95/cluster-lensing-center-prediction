...
from util.a1689_coords_conversion import xy_to_radec
...
# DATASET = 'a1689_fits'
DATASET = 'a1689_all_fits'
# DATASET = 'a1689_all_fits'

# Plot / output toggles -----------------------------------------------------
BOX_FILTER = True          # restrict intersections to quad bounding box
WEIGHT_INTERS = False      # weight intersections by distance error
...
def main():
    systems = du.quad_datasets.get(DATASET, du.a1689_perfect_four)

    # Static graph limits (pixels, re-centered at 2048)
    graph_limits = np.array([[-2000, -1300], [2000, 1600]])
    gx_min, gx_max = graph_limits[:, 0]

    graph = gu.Graph(
        title='Abell 1689 Quads',
    ...
    for idx, system in enumerate(systems):
        name = system.name
        if name == '19 FITS':
            print(f'Skipping quad {name}')
            continue
        images = system.images.copy() - CENTER_OFFSET        # recenter
        ...
        print(f'Quad {name} center (pixels): {ell_center[0] + CENTER_OFFSET}, {ell_center[1] + CENTER_OFFSET}')
        dist, nearest_point = mu.point_to_hyperbola_distance(
            ell_center[0], ell_center[1], hyp, return_nearest_point=True
        )
        print(f'Quad {name} nearest point hyperbola: {nearest_point[0] + CENTER_OFFSET}, {nearest_point[1] + CENTER_OFFSET}')
        fit_errors.append(
            dist
        )
        ...
        if PLOT_ELLIPSES:
            ex1, ey1, ex2, ey2 = mu.generate_conic_linspaces(
                ell_coeffs, 2000, gx_min, gx_max
            )
            graph.add_trace(ex1, ey1, 'ellipse',
                            color_idx=idx, line_width=LINE_WIDTH, showlegend=False)
            graph.add_trace(ex2, ey2, 'ellipse',
                            color_idx=idx, line_width=LINE_WIDTH, showlegend=False)
    ...
    xray_center = np.array([2871.4771,2600.0484]) - CENTER_OFFSET
    graph.add_trace(xray_center[0], xray_center[1],
                    'lens', name='X-ray Center',
                    showlegend=True)
    ...
    graph.fig.update_layout(
        title=f'Abell 1689 Quads (Predicted Center at {np.round(predicted, 2)})'
    )
