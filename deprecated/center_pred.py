...
def main():
    ...
    for idx, system in enumerate(systems):
        name = system.name
        print(f'Processing quad {name} ({idx + 1}/{len(systems)})')
        if name == '4':
    ...
    graph.fig.update_layout(
        title=f'J2325_4111 Quads (Predicted Center at {np.round(predicted, 2)})'
    )
